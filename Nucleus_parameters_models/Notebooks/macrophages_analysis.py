...
def make_nucleus_classifier(nucleus_dataset, confusion_matrix=False, no_NT=False):
    """
    Create and train an XGBoost classifier for nucleus dataset.

    Args:
        nucleus_dataset (DataFrame): Input dataset containing features and labels.

    Returns:
        tuple: A tuple containing confusion matrix, trained classifier, and test data.
    """
    ...
    # Fit the pipeline to the training data and make prediction
...
def make_multiclass_roc(pipe, test_data):
    ...
    # Binarize the labels into one-hot encoded format
